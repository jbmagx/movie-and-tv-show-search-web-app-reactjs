...
export default function Movies() {
    ...
    const fetchMoviesOrTVShows = async (url, page) => {
        try {
            ...
            setTotalPages(fetchedMovies.total_pages > 500 ? 500 : fetchedMovies.total_pages);
    ...
    useEffect(() => {    
        if (!isLoading && searchKeywords === '') {
            fetchMoviesOrTVShows('https://api.themoviedb.org/3/discover/movie?include_adult=false&include_video=false&language=en-US&sort_by=popularity.desc&page=', 1);
        }
    }, [searchKeywords]);
    ...
    return (
        <div className="p-2 xs:p-0 xxs:p-0">
            ...
            <div className="flex flex-wrap gap-5 xs:gap-3 xxs:gap-3">
                {
                    (movies.length === 0 && searchKeywords !== '') 
                    ?   <div className='mx-auto my-32 text-center'>There are no movies that matched your query.</div>
                    :   <>
                            {
                                movies.map((movie) => (
                                    <div
