...
export default function MoviesSnippet() {
...
export default function Movies() {
    ...
    const [mainURL, setMainURL] = useState('https://api.themoviedb.org/3/discover/movie?include_adult=false&include_video=false&language=en-US&sort_by=popularity.desc&page=');
        ...
        >
            ...
            >
                <div className="min-w-[80px] w-[148px] md:w-[134px] sm:w-[112px] xs:w-[94px] xxs:w-[80px] h-full">
                    <img
                        src={\`\${movie.poster_path === null ? NoImage : \`https://image.tmdb.org/t/p/original/\${movie.poster_path}\`}\`}
            ...
            <div className="flex flex-wrap flex-col justify-center gap-0 p-5 xs:p-3 xxs:p-3 overflow-hidden w-[calc(100%-148px)] max-h-[224px] md:w-[calc(100%-134px)] md:max-h-[201px] sm:w-[calc(100%-112px)] sm:max-h-[168px] xs:w-[calc(100%-94px)] xs:max-h-[141px] xxs:w-[calc(100%-80px)] xxs:max-h-[122px]">
                <div className="flex flex-wrap">
                    ...
                    <div className="w-[calc(100%-56px)] sm:w-[calc(100%-48px)] xs:w-full xxs:w-full flex flex-wrap flex-col justify-center gap-0 ml-2 xs:ml-0 xxs:ml-0">
                        <Link
                            ...
                            target="_blank"
                        >
                            <h2 className="w-full max-h-10 self-center line-clamp-1 xs:line-clamp-2 xxs:line-clamp-1 text-base sm:text-sm xs:text-sm xxs:text-sm font-semibold text-black tracking-[-0.25px]">{\`\${movie.title}\`}</h2>
                        </Link>
                        <small className="w-full self-center text-sm sm:text-xs xs:text-xs xxs:text-xs text-default-500 tracking-[-0.25px]">{\`\${movie.release_date}\`}</small>
                    </div>
                </div>
                <ScrollShadow
                    hideScrollBar
                ...
                >
                    <p className="w-full self-center text-black text-sm sm:text-tiny xs:text-tiny xxs:text-tiny">{\`\${movie.overview}\`}</p>
                </ScrollShadow>
            </div>
        </div>
    ));
    ...
    return (
        <div className="p-2 xs:p-0 xxs:p-0">
            <div className="w-[932px] md:w-[680px] sm2:w-full sm:w-full xs:w-full xxs:w-full mx-auto mb-5 xs:mb-3">
                <SearchInput search={'movie'} searchKeywords={searchKeywords} setSearchKeywords={setSearchKeywords} fetchMoviesOrTVShows={fetchMoviesOrTVShows} mainURL={mainURL} setMainURL={setMainURL} setPageNumber={setPageNumber} />
            </div>

            <div className="flex flex-wrap gap-5 xs:gap-3 xxs:gap-3">
                {(movies.length === 0 && searchKeywords !== '') ? <div className='mx-auto my-32 text-center'>There are no movies that matched your query.</div> : listMovies}
            </div>
    ...
    return (
        <div id='code-container' className='max-w-[702px] sm:max-w-[574px] mx-auto rounded-[5px] border border-[#656565]'>
            <div id='code-container-header' className='flex flex-wrap justify-between items-center px-5 h-10 bg-[#343A46] rounded-tl-[4px] rounded-tr-[4px] border-b border-[#656565]'>
                <div className='font-["Monaco",_sans-serif] text-sm text-white tracking-[-0.75px]'>Movies.jsx</div>
                <Button
